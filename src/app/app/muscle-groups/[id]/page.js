...
export default function MuscleGroupDetailPage() {
  ...
  useEffect(() => {
    if (doc) {
      setForm({ name: doc.name ?? "", thumbnail: doc.thumbnail ?? "" });
    }
  }, [doc]);
  ...
  if (error || !doc) {
    return (
      <div>
        <Button as={Link} href="/app/muscle-groups" variant="light" size="sm">
          ← Muscle groups
        </Button>
    ...
    <div>
      <div className="mb-6 flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-semibold text-foreground">{doc.name || "Muscle group"}</h1>
        </div>
        ...
        <CardBody className="gap-6">
          <form onSubmit={handleSave} className="flex flex-col gap-6">
            ...
            <div className="flex gap-2">
              ...
              <Button as={Link} href="/app/muscle-groups" variant="flat">
                Cancel
              </Button>
            </div>
          </form>
