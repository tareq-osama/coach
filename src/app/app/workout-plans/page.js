...
export default function WorkoutPlansPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (!user?.$id) {
      setFormError("You must be signed in to create or edit workout plans.");
      return;
    }
    setSaving(true);
    try {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-12">
        ...
        <p className="text-default-500">Loading workout plans…</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <Card className="border-danger-200 bg-danger-50/20">
        <CardBody>
          <p className="font-medium text-danger-700">Error loading workout plans</p>
  ...
  return (
    <div>
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-foreground">Workout Plans</h1>
          <p className="mt-2 text-default-500">Create and assign workout routines.</p>
        </div>
      ...
      <Card className="mt-6">
        {plans.length === 0 ? (
          <CardBody>
            <EmptyState pathname="/app/workout-plans" message="No workout plans yet. Add one to get started." className="py-12" />
          </CardBody>
        ) : (
            ...
            {plans.map((p) => (
              <li key={p.$id} className="flex items-center gap-4 px-4 py-3 sm:px-6">
                ...
                <div className="min-w-0 flex-1">
                  <Link href={`/app/workout-plans/${p.$id}`} className="font-medium text-foreground hover:underline">
                    {p.name}
                  </Link>
                  {p.description && (
                ...
                <div className="flex shrink-0 gap-2">
                  <Button as={Link} href={`/app/workout-plans/${p.$id}`} size="sm" variant="flat">
                    View / Edit
                  </Button>
      ...
      <Modal isOpen={isFormOpen} onClose={onFormClose}>
        <ModalContent>
          <form onSubmit={handleSubmit}>
            <ModalHeader>{editingId ? "Edit workout plan" : "Add workout plan"}</ModalHeader>
