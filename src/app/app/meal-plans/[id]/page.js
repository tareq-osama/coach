...
export default function MealPlanDetailPage() {
  ...
  const { data: junctionDocs, refetch: refetchJunction } = useGymList("meal-plan-modules", {
    queryParams: id ? { meal_plan_id: id } : undefined,
  });
  const { data: allMealModules } = useGymList("meals-modules");
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    setAssignedModules(
      sorted.map((d) => ({
        $id: d.$id,
        meal_module_id: d.meal_module_id,
  ...
  const moduleById = useMemo(() => {
    const map = {};
    (allMealModules ?? []).forEach((m) => (map[m.$id] = m));
    return map;
  }, [allMealModules]);

  const availableModules = useMemo(() => {
    const assignedIds = new Set(assignedModules.map((a) => a.meal_module_id));
    return (allMealModules ?? []).filter((m) => !assignedIds.has(m.$id));
  }, [allMealModules, assignedModules]);
  ...
  async function saveAssignedModules() {
    ...
    try {
      ...
      for (const row of assignedModules.filter((r) => r.$id)) {
        await fetch(`/api/gym/meal-plan-modules/${row.$id}`, {
          method: "DELETE",
          headers,
        });
      }
      for (let i = 0; i < assignedModules.length; i++) {
        ...
        await fetch("/api/gym/meal-plan-modules", {
          method: "POST",
          headers: { "Content-Type": "application/json", ...headers },
          body: JSON.stringify({
            meal_plan_id: id,
            meal_module_id: row.meal_module_id,
            sort_order: i,
          }),
        });
      }
  ...
  function addModule(moduleId) {
    if (assignedModules.some((a) => a.meal_module_id === moduleId)) return;
    setAssignedModules((prev) => [
      ...prev,
      { meal_module_id: moduleId, sort_order: prev.length },
    ]);
  }
  ...
  if (error || !doc) {
    return (
      <div>
        <Button as={Link} href="/app/meal-plans" variant="light" size="sm">
          ← Meal plans
        </Button>
    ...
    <div>
      <div className="mb-6 flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-semibold text-foreground">{doc.name || "Meal plan"}</h1>
        </div>
        ...
        <CardBody className="gap-6">
          <form onSubmit={handleSave} className="flex flex-col gap-6">
            ...
            <div className="flex gap-2">
              ...
              <Button as={Link} href="/app/meal-plans" variant="flat">
                Cancel
              </Button>
            </div>
          </form>
      ...
      <Card className="mt-6">
        <CardBody className="gap-4">
          <h2 className="text-lg font-semibold">Assigned meal modules</h2>
          {modulesError && (
            ...
            {assignedModules.map((row, index) => (
              ...
              >
                <span className="order-1 min-w-0 flex-1 font-medium">
                  {moduleById[row.meal_module_id]?.name ?? row.meal_module_id}
                </span>
