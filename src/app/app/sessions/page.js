"use client";

import { useGymList } from "../use-gym-list";
import { Card, CardBody, Spinner } from "@heroui/react";

function formatDate(iso) {
  if (!iso) return "—";
  try {
    const d = new Date(iso);
    return d.toLocaleDateString(undefined, { dateStyle: "medium" }) + " " + d.toLocaleTimeString(undefined, { timeStyle: "short" });
  } catch {
    return iso;
  }
}

export default function SessionsPage() {
  const { data: sessions, loading, error } = useGymList("sessions");
  const { data: members } = useGymList("members");
  const { data: plans } = useGymList("workout-plans");
  const memberMap = Object.fromEntries((members ?? []).map((m) => [m.$id, m.name || m.email || "—"]));
  const planMap = Object.fromEntries((plans ?? []).map((p) => [p.$id, p.name]));

  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-12">
        <Spinner size="lg" color="primary" />
        <p className="text-default-500">Loading sessions…</p>
      </div>
    );
  }

  if (error) {
    return (
      <Card className="border-danger-200 bg-danger-50/20">
        <CardBody>
          <p className="font-medium text-danger-700">Error loading sessions</p>
          <p className="mt-1 text-sm text-danger-600">{error}</p>
        </CardBody>
      </Card>
    );
  }

  return (
    <div>
      <h1 className="text-2xl font-semibold text-foreground">Sessions</h1>
      <p className="mt-2 text-default-500">Schedule and track training sessions.</p>
      <Card className="mt-6">
        {sessions.length === 0 ? (
          <CardBody className="text-center py-12 text-default-500">No sessions yet.</CardBody>
        ) : (
          <ul className="divide-y divide-default-200">
            {sessions.map((s) => (
              <li key={s.$id} className="px-4 py-3 sm:px-6">
                <p className="font-medium text-foreground">
                  {formatDate(s.session_date)} — {memberMap[s.member_id] ?? s.member_id}
                </p>
                {s.workout_plan_id && (
                  <p className="mt-0.5 text-sm text-default-500">
                    Plan: {planMap[s.workout_plan_id] ?? s.workout_plan_id}
                  </p>
                )}
                {s.notes && <p className="mt-0.5 text-sm text-default-500">{s.notes}</p>}
              </li>
            ))}
          </ul>
        )}
      </Card>
    </div>
  );
}
